(ns freightops.operation
  "OperationActor -- one freight operation = one supervised actor run,
  expressed as a langgraph-clj StateGraph. The advisor (FreightOps-LLM)
  is sealed into a single node (:advise); its proposal is ALWAYS
  routed through the Freight Governor (:govern) and the rollout phase
  ...
  One graph run = one freight operation (intake -> advise -> govern ->
  ...
  decision to a human operator (carrier dispatcher/operator). The
  approver resumes with `{:approval {:status :approved}}` (or
  :rejected). `:shipment/dispatch`/`:consignment/settle` ALWAYS reach
  this node when the governor is clean -- see `freightops.phase`."
  (:require [langgraph.graph :as g]
            [langgraph.checkpoint :as cp]
            [freightops.freightopsllm :as freightopsllm]
            [freightops.governor :as governor]
...
(defn build
  ...
  `freightops.store/Store`).
  opts:
    :advisor      -- a `freightops.freightopsllm/Advisor` (default: mock-advisor)
    :checkpointer -- langgraph checkpointer (default: in-mem)"
  [store & [{:keys [advisor checkpointer]
             :or   {advisor      (freightopsllm/mock-advisor)
  ...
  (-> (g/state-graph
      ...
      ;; FreightOps-LLM inference (the contained intelligence node) -- proposal only.
      (g/add-node :advise
        (fn [{:keys [request]}]
          (let [p (freightopsllm/-advise advisor store request)]
            {:proposal p :audit [(freightopsllm/trace request p)]})))

      ;; Freight Governor -- independent censor (separate system than the LLM).
      (g/add-node :govern
